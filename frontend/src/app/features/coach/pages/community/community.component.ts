...
export class CommunityComponent implements OnInit {
  ...
  setupCurrentUser() {
    ...
    if (user && user.id) {
      ...
      if (!this.currentUser.name.trim()) this.currentUser.name = 'Coach';
  ...
  loadCommunityAndPosts() {
    ...
    this.coachService.getCoachByUserId(user.id).subscribe({
      next: (coachProfile) => {
        if (coachProfile && coachProfile.clubId) {
          if (coachProfile.id) {
            this.loadCoachClients(coachProfile.id);
          }
          this.communityService.getByClubId(coachProfile.clubId).subscribe({
            next: (community: CommunityResponse) => {
              this.community = community;
              this.loadPosts();
            },
            error: (err: any) => {
              console.error('Failed to load club community, falling back to all communities', err);
  ...
  loadCoachClients(coachId: number) {
    this.clientService.getClientsByCoachId(coachId).subscribe({
      next: (clients) => {
      ...
      error: (err) => console.error('Failed to load coach clients', err)
    });
  }

  fallbackToAnyCommunity() {
    this.communityService.getAll().subscribe({
      next: (communities: CommunityResponse[]) => {
    ...
    this.postService.getByCommunityId(this.community.id).subscribe({
      next: (dbPosts: PostResponse[]) => {
        ...
        const mappedPosts: Post[] = dbPosts.map(p => {
          ...
          if (hashtags) {
            postObj.tags = hashtags.map(t => t.substring(1));
          }
  ...
  submitPost(): void {
    ...
    // Auto-append tags as hashtags to the content
    ...
    this.postService.create(this.currentUser.id, request).subscribe({
      next: () => {
        ...
        this.showNotification('Votre publication a été partagée !', 'success');
        this.loadPosts();
      },
      error: (err: any) => {
        this.isLoading = false;
        this.showNotification('Erreur de publication : ' + (err.error?.message || err.message), 'error');
      }
    });
  }
  ...
  deletePost(post: Post): void {
    this.postService.delete(post.id).subscribe({
      next: () => {
        ...
        this.showNotification('Publication supprimée.', 'success');
      },
      error: (err: any) => {
